...
function EavesDrop:SendLineToChat(text)
	...
	if (GetNumRaidMembers() >= 1) then 
		SendChatMessage(timestring.." - "..logstring ,"RAID" , nil);
	elseif (GetNumPartyMembers() >= 1) then
		SendChatMessage(timestring.." - "..logstring ,"PARTY" , nil);
	else
		SendChatMessage(timestring.." - "..logstring ,"SAY" , nil);
	end
end
...
function EavesDrop:SendHistoryToChat(text, part)
	...
	if part == 1 and not nonormal then
		if (GetNumRaidMembers() >= 1) then 
			SendChatMessage(firstlinetime.." - "..firstlinetext ,"RAID" , nil);
		elseif (GetNumPartyMembers() >= 1) then
			SendChatMessage(firstlinetime.." - "..firstlinetext ,"PARTY" , nil);
		else
			SendChatMessage(firstlinetime.." - "..firstlinetext ,"SAY" , nil);
		end
	elseif part == 2 and not nocrit then
		if (GetNumRaidMembers() >= 1) then 
			SendChatMessage(secondlinetime.." - "..secondlinetext ,"RAID" , nil);
		elseif (GetNumPartyMembers() >= 1) then
			SendChatMessage(secondlinetime.." - "..secondlinetext ,"PARTY" , nil);
		else
			SendChatMessage(secondlinetime.." - "..secondlinetext ,"SAY" , nil);
		end
	end
end
